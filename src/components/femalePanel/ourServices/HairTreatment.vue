<template>
    <div class="flex flex-col min-h-screen">
      ...
      <div class="flex-grow mt-14">
        <div class="bg-white">
          ...
          >
            <p class="text-lg font-bold tracking-tight text-pinkColor">
              Book Appointment
            </p>
            <div class="mt-4 grid grid-cols-1 gap-x-4 gap-y-5 sm:grid-cols-1 md:grid-cols-2 md:gap-x-2 lg:grid-cols-2 xl:gap-x-14">
              <div class="">
                <img
                  alt="ecommerce"
                  class="lg:w-full w-full lg:h-auto h-64 object-cover object-center rounded shadow-lg"
                  :src="HairTreatmentData[0].img"
                />
              </div>
              <div>
                <h1 class="text-goldColor text-3xl title-font font-medium mb-4">
                  {{ HairTreatmentData[0].title }}
                </h1>
                <p class="leading-relaxed mb-4">{{ HairTreatmentData[0].description }}</p>
                <div class="w-full">
                  <table class="w-full border-collapse bg-gray-100 text-left">
                    <thead class="bg-pinkColor text-white">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      <tr
                        ...
                        v-for="item in HairTreatmentData[0].KerateinData"
                        :key="item.id"
                      >
                ...
                <div class="">
                  <table class="w-full border-collapse bg-gray-100 text-left">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      <tr
                        class="cursor-pointer hover:bg-gray-50"
                        v-for="item in HairTreatmentData[1].ProteinData"
                        :key="item.id"
                      >
                        <td class="py-2 px-4">{{ item.name }}</td>
                        <td class="py-2 px-4 text-center">
  ...
  <script>
  import { HairTreatmentData } from '@/db/female-db'
  ...
  export default {
    ...
    data() {
      return {
        HairTreatmentData:HairTreatmentData
      };
    },
    components: {
      Header,
