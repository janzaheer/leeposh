<template>
            ...
            <div class="mt-4 grid grid-cols-1 gap-x-4 gap-y-5 sm:grid-cols-1 md:grid-cols-2 md:gap-x-2 lg:grid-cols-2 xl:gap-x-14">
              <div class="">
                <img
                  ...
                  :src="PedicureData[0].img"
                />
              </div>
              <div>
                <h1 class="text-goldColor text-3xl title-font font-medium mb-4">
                  {{ PedicureData[0].title }}
                </h1>
                <p class="leading-relaxed mb-4">{{ PedicureData[0].description }}</p>
                <div class="w-full">
                  <table class="w-full border-collapse bg-gray-100 text-left">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      <tr
                        ...
                        v-for="item in PedicureData[0]?.PediData"
                        :key="item.id"
                      >
  ...
  <script>
  import { PedicureData } from '@/db/female-db'
