
export const HairCareData = [
    {
        id: 1,
        title: "Cut's & Styling",
        description: "A classic men's haircut, often referred to as a standard or normal cut, where the hair is trimmed and shaped to maintain a neat and clean appearance.",
        img: "https://plus.unsplash.com/premium_photo-1665203406912-4fbd99560a90?q=80&w=3270&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        data: [
            {
                name: "Simple Haircut",
                price: '800'
            },
            {
                name: "Face Haircut",
                price: '1000'
            },
            {
                name: "Head Shave",
                price: '400'
            },
            {
                name: "Styling",
                price: '500'
            },
            {
                name: "Washt",
                price: '800'
            },
        ]
    },
    {
        id: 2,
        title: "Hair Treatment",
        TreatmentData: [
            {
                name: "Instant Hair Straightening",
                price: '5000'
            },
            {
                name: "Simple Hair Mask",
                price: '3000 and 2000'
            },
            {
                name: "Hair Polishing",
                price: '2500'
            }
        ]
    },
    ,
    {
        id: 3,
        title: "Keratian",
        KeratianData: [
            {
                name: "Level 1",
                price: '8000'
            },
            {
                name: "Level 2",
                price: '10000'
            },
            {
                name: "Level 3",
                price: '15000'
            }
        ]
    }
]
export const FaceCareData = [
    {
        id: 1,
        title: "Bread & Shave",
        description: "A classic men's haircut, often referred to as a standard or normal cut, where the hair is trimmed and shaped to maintain a neat and clean appearance.",
        img: "https://plus.unsplash.com/premium_photo-1665203406912-4fbd99560a90?q=80&w=3270&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        BreadData: [
            {
                name: "Simple Trimming",
                price: '300'
            },
            {
                name: "Wet Shave",
                price: '500'
            },
            {
                name: "Wet Shave Package",
                price: '800'
            },
        ]
    },
    {
        id: 2,
        title: "Facials",
        FacialstData: [
            {
                name: "Janssen",
                price: '6500'
            },
            {
                name: "The Body Shop",
                price: '6000'
            },
            {
                name: "Hydia Facial",
                price: '5500'
            }
            ,
            {
                name: "Charcoal",
                price: '3000'
            }
        ]
    },
    {
        id: 3,
        title: "Cleaning",
        CleaningData: [
            {
                name: "Janssen",
                price: '6500'
            },
            {
                name: "THe Body Shop",
                price: '3000'
            },
            {
                name: "Face Polish",
                price: '2500'
            },
            {
                name: "Makeover",
                price: '4000'
            },
            {
                name: "Simple Touch",
                price: '2000'
            }
        ]
    },
    {
        id: 4,
        title: "Threading",
        ThreadingData: [
            {
                name: "Full Face",
                price: '600'
            },
            {
                name: "EyeBrow",
                price: '300'
            },
            {
                name: "Cheeks",
                price: '300'
            }
        ]
    },
    {
        id: 4,
        title: "Face Masks",
        MasksData: [
            {
                name: "Golden Mask",
                price: '500'
            },
            {
                name: "Black Mask",
                price: '300'
            },
            {
                name: "Mud Mask",
                price: '300'
            }
            ,
            {
                name: "Nose Mask",
                price: '400'
            }
        ]
    },
]
export const CoulouringData = [
    {
        id: 1,
        title: "Colouring & Texturing",
        description: "A classic men's haircut, often referred to as a standard or normal cut, where the hair is trimmed and shaped to maintain a neat and clean appearance.",
        img: "https://plus.unsplash.com/premium_photo-1665203406912-4fbd99560a90?q=80&w=3270&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        TexturingData: [
            {
                name: "Loreal",
                price: '2500'
            },
            {
                name: "Keune",
                price: '2000'
            },
            {
                name: "Garnier",
                price: '1600'
            },
            {
                name: "Just For Men",
                price: '2500'
            },
            {
                name: "Hight Light",
                price: '6000'
            },
            {
                name: "Low Lights",
                price: '5000'
            },
            {
                name: "Glossing",
                price: '2000'
            },
            {
                name: "Beard Colouring",
                price: '800'
            },
            {
                name: "Colour Application",
                price: '1000'
            },
        ]
    },
]
export const WaxingData = [
    {
        id: 1,
        title: "Face Wax",
        description: "A classic men's haircut, often referred to as a standard or normal cut, where the hair is trimmed and shaped to maintain a neat and clean appearance.",
        img: "https://plus.unsplash.com/premium_photo-1665203406912-4fbd99560a90?q=80&w=3270&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        FaceData: [
            {
                name: "Full Face wax with cooling mask",
                price: '1200'
            },
            {
                name: "Full Face wax",
                price: '1000'
            },
            {
                name: "Cheek wax",
                price: '500'
            },
            {
                name: "Nose wax",
                price: '400'
            },
            {
                name: "Ear wax",
                price: '400'
            },
            {
                name: "Forehead wax",
                price: '500'
            }
        ]
    },
    {
        id: 2,
        title: "Body Wax",
        BodyData: [
            {
                name: "Full Arms wax (one side)",
                price: '1600'
            },
            {
                name: "Full Legs wax (one side)",
                price: '1600'
            },
            {
                name: "Cheast",
                price: '2000'
            },
            {
                name: "Back",
                price: '3000'
            },
            {
                name: "Half Arm (single)",
                price: '800'
            },
            {
                name: "Half legs (single)",
                price: '800'
            },
            {
                name: "Stomach",
                price: '2000'
            }
        ]
    }
]
export const MassageData = [
    {
        id: 1,
        title: "Massage",
        description: "Relaxing and therapeutic massage services for stress relief and muscle relaxation.",
        img: "https://plus.unsplash.com/premium_photo-1665203406912-4fbd99560a90?q=80&w=3270&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        MassageBodyData: [
            {
                name: "Head Massage",
                price: '400'
            },
            {
                name: "Shoulder Massage",
                price: '300'
            },
            {
                name: "Foot Massage",
                price: '500'
            },
            {
                name: "Full Leg Massage",
                price: '1500'
            },
            {
                name: "Full Back Massage",
                price: '1500'
            },
            {
                name: "Full Body Massage",
                price: '4000'
            }
        ]
    },
    {
        id: 2,
        title: "Luxury Service",
        LuxuryData: [
            {
                name: "Private Room",
                price: '1000'
            },
            {
                name: "Body Scrubbing",
                price: '6000'
            },
            {
                name: "Body Polishing",
                price: '6500'
            }
        ]
    },
    {
        id: 3,
        title: "Hands & Feet",
        HandsData: [
            {
                name: "Whitening Manicure",
                price: '2000'
            },
            {
                name: "Whitening Pedicure",
                price: '2000'
            },
            {
                name: "Simple Manicure",
                price: '1500'
            },
            {
                name: "Simple Pedicure",
                price: '1500'
            },
            {
                name: "Praffin Manicure",
                price: '2500'
            },
            {
                name: "Praffin Pedicure",
                price: '2500'
            },
            {
                name: "Nails Cutting / Filing",
                price: '500'
            },
            {
                name: "Cuticles Removing",
                price: '500'
            },
            {
                name: "Nail Buffing",
                price: '500'
            }
        ]
    }
]
export const GroomingData = [
    {
        id: 1,
        title: "Two Days Package",
        description: "A classic men's haircut, often referred to as a standard or normal cut, where the hair is trimmed and shaped to maintain a neat and clean appearance.",
        img: "https://plus.unsplash.com/premium_photo-1665203406912-4fbd99560a90?q=80&w=3270&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        price: 12000
    },
    {
        TwoDayData: [
            {
                name: "Haircut",
            },
            {
                name: "Wet Shave",
            },
            {
                name: "Styling (both days)",
            },
            {
                name: "Dermacos Facial",
            },
            {
                name: "Makeover (one day)",
            },
            {
                name: "Golden Mask",
            },
            {
                name: "Simple Mani/Pedi",
            },
            {
                name: "Waxing / Threading",
            }
        ]
    },
    {
        id: 2,
        title: "Three Days Package",
        price: 17000,
        ThreeDayyData: [
            {
                name: "Haircut",
            },
            {
                name: "Wet Shave",
            },
            {
                name: "Simple Trimming",
            },
            {
                name: "Whitening Mani/Pedi",
            },
            {
                name: "The Body Shop Facial",
            },
            {
                name: "Waxing / Threading",
            },
            {
                name: "Styling (All Three Days)",
            },
            {
                name: "Makeover 1 Day & Simple Touch 1 Day",
            }
        ]
    },
    {
        id: 3,
        title: "Four Days Package",
        price: 22000,
        FourDayData: [
            {
                name: "Haircut",
            },
            {
                name: "Wet Shave",
            },
            {
                name: "Simple Trimming",
            },
            {
                name: "Whitening Mani/Pedi",
            },
            {
                name: "Jassen Facial",
            },
            {
                name: "Makeover 2 Days",
            },
            {
                name: "Waxing / Threading",
            },
            {
                name: "Styling (All Four Days)",
            },
            {
                name: "Led Steamer / Face Mask",
            },
            {
                name: "Black / White Head Remover",
            },
        ]
    }
]